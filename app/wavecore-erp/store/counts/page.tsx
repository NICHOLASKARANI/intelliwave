...
export default function CountsPage() {
  ...
  const handleDownloadPDF = () => {
    const content = ['WaveCore ERP - Stock Counts', '='.repeat(50), `Products: ${products.length}`, '', '© 2026 IntelliWavve'].join('\n')
    const blob = new Blob([content], { type: 'application/pdf' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = 'counts.pdf'; a.click()
  }
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950">
      ...
      <main className="max-w-4xl mx-auto p-4 lg:p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold flex items-center gap-2"><ClipboardList className="w-6 h-6 text-purple-500" /> Stock Counts</h1>
        ...
        {loading ? <Loader2 className="w-8 h-8 animate-spin mx-auto" /> : (
          <div className="bg-white dark:bg-neutral-900 rounded-2xl border p-6 text-center">
            ...
            <p className="text-sm text-muted-foreground">Products to count</p>
          </div>
        )}
      </main>
    </div>
