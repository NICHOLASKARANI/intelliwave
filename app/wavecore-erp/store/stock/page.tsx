...
import { Loader2, Trash2, Search, Package, Printer, CheckCircle2, AlertTriangle, XCircle, Boxes, ArrowDownUp } from 'lucide-react'
...
export default function StockPage() {
  ...
  const downloadPdf = (id: string) => {
    ...
    window.open(`/api/wavecore/store/stock/${id}/pdf`, '_blank')
  }
  ...
  const totalStockUnits = Number(stats.totalStockUnits || 0)
      ...
      <main className="max-w-6xl mx-auto p-3 sm:p-4 lg:p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Boxes className="w-6 h-6 text-blue-500" /> Stock Management ({totalProducts})
          </h1>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <button onClick={() => setActiveView('in')}
            className={`p-5 rounded-2xl text-white text-center transition-all hover:shadow-lg ${activeView === 'in' ? 'ring-4 ring-green-300' : ''}`}
          ...
          <button onClick={() => setActiveView('out')}
            className={`p-5 rounded-2xl text-white text-center transition-all hover:shadow-lg ${activeView === 'out' ? 'ring-4 ring-red-300' : ''}`}
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-6 mb-6 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm opacity-80">Total Stock Units</p>
                <p className="text-3xl font-bold">{totalStockUnits.toLocaleString()}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-sm opacity-80">Stock Value</p>
              <p className="text-3xl font-bold">KSh {products.reduce((sum, p) => sum + (Number(p.sellingPrice || 0) * Number(p.stock_level || 0)), 0).toLocaleString()}</p>
            </div>
          </div>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-neutral-50 dark:bg-neutral-800">
                <tr>
                  ...
                  <th className="text-right p-4 text-sm">Stock Level</th>
              ...
              <tbody>
                {filtered
                  .filter(p => {
                    if (activeView === 'in') return p.stockStatus === 'IN_STOCK'
                    if (activeView === 'low') return p.stockStatus === 'LOW_STOCK'
                    if (activeView === 'out') return p.stockStatus === 'OUT_OF_STOCK'
                    return true
                  })
                  .map((product) => {
                    ...
                    return (
                      <tr key={product.id} className="border-t hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-colors">
                        ...
                        <td className="p-4 text-right">
                          <span className={`font-bold ${stockLevel === 0 ? 'text-red-600' : stockLevel < 10 ? 'text-yellow-600' : 'text-green-600'}`}>
                            {stockLevel}
                          </span>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4 text-right font-bold text-blue-600">KSh {stockValue.toLocaleString()}</td>
                        <td className="p-4">
