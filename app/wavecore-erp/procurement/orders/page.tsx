...
import { Package, Plus, Download, Loader2, Trash2, Search, FileText } from 'lucide-react'
...
export default function PurchaseOrdersPage() {
  ...
  const [formData, setFormData] = useState({ supplierName: '', amount: '', status: 'PENDING' })
  ...
  const fetchOrders = async () => {
    try {
      const res = await fetch('/api/wavecore/purchase-orders')
  ...
  const addOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setFormData({ supplierName: '', amount: '', status: 'PENDING' })
        setShowForm(false)
        fetchOrders()
      }
    } catch (error) {
  ...
  const deleteOrder = async (id: string) => {
    ...
    try {
      await fetch(`/api/wavecore/purchase-orders?id=${id}`, { method: 'DELETE' })
      fetchOrders()
    } catch (error) {
      ...
      <html>
        ...
        <body>
          ...
          <table>
            ...
            <tbody>
              ${orders.map(o => `<tr><td>${o.supplierName}</td><td>KSh ${o.amount}</td><td>${o.status}</td><td>${new Date(o.createdAt).toLocaleDateString()}</td></tr>`).join('')}
            </tbody>
          </table>
      ...
      <main className="max-w-7xl mx-auto p-4 lg:p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Package className="w-6 h-6 text-pink-500" /> Purchase Orders ({orders.length})
          </h1>
        ...
        {showForm && (
          <form onSubmit={addOrder} className="bg-white dark:bg-neutral-900 rounded-2xl border p-6 mb-6">
            ...
            <button type="submit" className="mt-4 px-6 py-2 rounded-xl bg-emerald-600 text-white">Create PO</button>
          </form>
        )}
        ...
        <div className="relative mb-6">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            className="pl-9 pr-4 py-2.5 rounded-xl border w-full" placeholder="Search purchase orders..." />
        </div>
              ...
              <tbody>
                {filtered.map(order => (
                  <tr key={order.id} className="border-t">
                    <td className="p-4 font-medium">{order.supplierName}</td>
                    <td className="p-4">KSh {order.amount}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4"><button onClick={() => deleteOrder(order.id)} className="text-red-500 hover:text-red-700"><Trash2 className="w-4 h-4" /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
