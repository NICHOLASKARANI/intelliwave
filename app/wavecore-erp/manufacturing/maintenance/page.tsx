...
export default function MaintenancePage() {
  ...
  useEffect(() => {
    fetch('/api/wavecore/manufacturing/maintenance')
      .then(r => r.json())
  ...
  const handleDownloadPDF = () => {
    const content = ['WaveCore ERP - Maintenance', '='.repeat(50), `Requests: ${requests.length}`, '', '© 2026 IntelliWavve'].join('\n')
    const blob = new Blob([content], { type: 'application/pdf' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = 'maintenance.pdf'; a.click()
  }
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950">
      ...
      <main className="max-w-4xl mx-auto p-4 lg:p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold flex items-center gap-2"><Wrench className="w-6 h-6 text-red-500" /> Maintenance ({requests.length})</h1>
