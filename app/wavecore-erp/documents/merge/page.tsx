...
export default function MergePage() {
  ...
  const handleMerge = () => {
    if (files.length < 2) {
      setError('Upload at least 2 images to merge')
      return
    }
  ...
  const downloadMerged = () => {
    ...
    a.href = mergedUrl
    a.download = 'merged-document.png'
  ...
  const handleDownloadReport = () => {
    const content = ['WaveCore ERP - Merge Report', '='.repeat(50), `Files Merged: ${files.length}`, '', ...files.map((f, i) => `${i+1}. ${f.name}`), '', '© 2026 IntelliWavve'].join('\n')
          ...
          <>
            <div className="bg-white dark:bg-neutral-900 rounded-2xl border p-6 mb-6">
              <label className="block border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer">
                ...
                <p className="font-medium">Upload JPG/PNG images to merge</p>
                <p className="text-xs text-muted-foreground mt-1">Select 2+ images</p>
