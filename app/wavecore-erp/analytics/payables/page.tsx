...
export default function payablesPage() {
  ...
  const handleDownloadPDF = () => {
    const content = ['WaveCore ERP - Payables', '='.repeat(50), 'Count: ' + (stats.accountsPayable || 0), '', '(c) 2026 IntelliWavve'].join('\n')
    const blob = new Blob([content], { type: 'application/pdf' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = 'payables.pdf'; a.click()
  }
      ...
      <main className="max-w-4xl mx-auto p-4 lg:p-8">
        <div className="rounded-3xl bg-gradient-to-br from-red-600 to-rose-700 p-6 mb-8">
          <div className="flex justify-between items-center">
            <h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingDown className="w-7 h-7" /> Payables</h1>
        ...
        {loading ? <Loader2 className="w-8 h-8 animate-spin mx-auto" /> : (
          <div className="p-6 rounded-2xl border bg-white dark:bg-neutral-900 text-center">
            <p className="text-4xl font-extrabold">{stats.accountsPayable || 0}</p>
