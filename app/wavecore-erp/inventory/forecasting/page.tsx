    ...
    <div className="min-h-screen bg-neutral-950">
      ...
      <div className="ml-64 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <button onClick={() => setActiveKpi('STOCKOUT')}
            className={'p-4 rounded-2xl bg-gradient-to-br from-red-600 to-rose-800 text-white shadow-lg text-left transition-all hover:shadow-xl ' + (activeKpi === 'STOCKOUT' ? 'ring-4 ring-red-300' : '')}>
            <AlertTriangle className="w-5 h-5 mb-2" />
            <p className="text-2xl font-bold">{summary.willStockout || 0}</p>
            <p className="text-xs opacity-80">Will Stockout</p>
          </button>
          <button onClick={() => setActiveKpi('UP')}
            className={'p-4 rounded-2xl bg-gradient-to-br from-green-600 to-emerald-800 text-white shadow-lg text-left transition-all hover:shadow-xl ' + (activeKpi === 'UP' ? 'ring-4 ring-green-300' : '')}>
            <TrendingUp className="w-5 h-5 mb-2" />
            <p className="text-2xl font-bold">{summary.trendingUp || 0}</p>
            <p className="text-xs opacity-80">Trending Up</p>
          </button>
          <button onClick={() => setActiveKpi('DOWN')}
            className={'p-4 rounded-2xl bg-gradient-to-br from-orange-600 to-amber-800 text-white shadow-lg text-left transition-all hover:shadow-xl ' + (activeKpi === 'DOWN' ? 'ring-4 ring-orange-300' : '')}>
          ...
          <button onClick={() => setActiveKpi('STABLE')}
            className={'p-4 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-800 text-white shadow-lg text-left transition-all hover:shadow-xl ' + (activeKpi === 'STABLE' ? 'ring-4 ring-blue-300' : '')}>
            ...
            <table className="w-full">
              <thead className="bg-neutral-800">
                <tr>
                  ...
                  <th className="text-right p-4 text-neutral-400 text-sm">Avg Daily</th>
                  <th className="text-right p-4 text-neutral-400 text-sm">7-Day Forecast</th>
                  <th className="text-right p-4 text-neutral-400 text-sm">30-Day Forecast</th>
              ...
              <tbody>
                {filtered.map((f: any) => (
                  <tr key={f.productId} className="border-t border-neutral-800 hover:bg-neutral-800/50 transition-colors">
                    <td className="p-4">
                      <p className="font-bold text-white">{f.name}</p>
                      <p className="text-xs text-neutral-400">{f.sku || 'N/A'}</p>
                    </td>
                    <td className="p-4 text-right text-white">{f.currentStock}</td>
                    <td className="p-4 text-right text-neutral-300">{f.avgDailyDemand}</td>
                    ...
                    <td className="p-4 text-right">
                      <span className={f.daysOfSupply < 14 ? 'text-red-400 font-bold' : 'text-green-400 font-bold'}>
                        {f.daysOfSupply === 999 ? '∞' : f.daysOfSupply}
                      </span>
                    </td>
                    <td className="p-4">
                      <span className={'px-2 py-1 rounded-full text-xs font-bold ' + 
                        (f.trend === 'UP' ? 'bg-green-900/50 text-green-300' :
                         f.trend === 'DOWN' ? 'bg-orange-900/50 text-orange-300' :
                    ...
                    <td className="p-4">
                      <span className={'px-2 py-1 rounded-full text-xs font-bold ' + 
                        (f.willStockout ? 'bg-red-900/50 text-red-300' : 'bg-green-900/50 text-green-300')}>
                        {f.willStockout ? 'STOCKOUT' : 'OK'}
                      </span>
                    </td>
                    <td className="p-4 text-center">
                      <button onClick={() => downloadPdf(f.productId)} className="p-2 rounded-lg bg-blue-900/50 text-blue-300 hover:bg-blue-800 transition-colors" title="Download PDF">
