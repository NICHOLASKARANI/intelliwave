    ...
    <div className="min-h-screen bg-neutral-950">
      ...
      <div className="ml-64 p-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2"><DollarSign className="w-6 h-6 text-emerald-500" /> Inventory Valuation</h1>
            <p className="text-sm text-neutral-400 mt-1">FIFO, Weighted Average, Standard Cost</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <button onClick={() => setActiveKpi('ALL')} className={"p-4 rounded-2xl bg-gradient-to-br from-emerald-600 to-green-800 text-white shadow-lg text-left " + (activeKpi === 'ALL' ? 'ring-4 ring-emerald-300' : '')}><BarChart3 className="w-5 h-5 mb-2" /><p className="text-2xl font-bold">{summary.totalProducts || 0}</p><p className="text-xs opacity-80">Products</p></button>
          <button onClick={() => setActiveKpi(activeKpi === 'UNITS' ? 'ALL' : 'UNITS')} className={"p-4 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-800 text-white shadow-lg text-left " + (activeKpi === 'UNITS' ? 'ring-4 ring-blue-300' : '')}><Package className="w-5 h-5 mb-2" /><p className="text-2xl font-bold">{summary.totalUnits || 0}</p><p className="text-xs opacity-80">Units</p></button>
          <button onClick={() => setActiveKpi(activeKpi === 'VALUE' ? 'ALL' : 'VALUE')} className={"p-4 rounded-2xl bg-gradient-to-br from-green-600 to-teal-800 text-white shadow-lg text-left " + (activeKpi === 'VALUE' ? 'ring-4 ring-green-300' : '')}><DollarSign className="w-5 h-5 mb-2" /><p className="text-2xl font-bold">KSh {(summary.totalValue || 0).toLocaleString()}</p><p className="text-xs opacity-80">Cost Value</p></button>
          <button onClick={() => setActiveKpi(activeKpi === 'PROFIT' ? 'ALL' : 'PROFIT')} className={"p-4 rounded-2xl bg-gradient-to-br from-orange-500 to-amber-700 text-white shadow-lg text-left " + (activeKpi === 'PROFIT' ? 'ring-4 ring-orange-300' : '')}><TrendingUp className="w-5 h-5 mb-2" /><p className="text-2xl font-bold">KSh {(summary.potentialProfit || 0).toLocaleString()}</p><p className="text-xs opacity-80">Profit</p></button>
        </div>
        <div className="relative mb-4"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-500" /><input type="text" value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 pr-4 py-2.5 rounded-xl bg-neutral-900 border border-neutral-800 text-white w-full focus:outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Search products..." /></div>
        {loading ? (
          ...
          <div className="bg-neutral-900 rounded-2xl border border-neutral-800 overflow-hidden shadow-xl">
            <table className="w-full">
              <thead className="bg-neutral-800"><tr><th className="text-left p-4 text-neutral-400 text-sm">Product</th><th className="text-right p-4 text-neutral-400 text-sm">Stock</th><th className="text-right p-4 text-neutral-400 text-sm">Cost</th><th className="text-right p-4 text-neutral-400 text-sm">Selling</th><th className="text-right p-4 text-neutral-400 text-sm">Total Cost</th><th className="text-right p-4 text-neutral-400 text-sm">Value</th><th className="text-right p-4 text-neutral-400 text-sm">Profit</th><th className="text-center p-4 text-neutral-400 text-sm">Actions</th></tr></thead>
              <tbody>
                {filtered.map((p: any) => (
