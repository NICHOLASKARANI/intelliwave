    ...
    <div className="min-h-screen bg-neutral-950">
      <div className="fixed left-0 top-0 h-full w-64 bg-neutral-900 border-r border-neutral-800 z-50">
        ...
        <nav className="p-4 space-y-2">
          ...
          <Link href="/wavecore-erp/inventory/reorder" className="flex items-center gap-3 p-3 rounded-xl bg-orange-600 text-white font-bold shadow-lg"><ShoppingCart className="w-5 h-5" /> Reorder</Link>
      ...
      <div className="ml-64 p-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2"><ShoppingCart className="w-6 h-6 text-orange-500" /> Reorder Automation</h1>
            <p className="text-sm text-neutral-400 mt-1">Auto-suggest purchase orders</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <button onClick={() => setActiveKpi(activeKpi === 'CRITICAL' ? 'ALL' : 'CRITICAL')} className={'p-4 rounded-2xl bg-gradient-to-br from-red-600 to-rose-800 text-white shadow-lg text-left ' + (activeKpi === 'CRITICAL' ? 'ring-4 ring-red-300' : '')}><AlertTriangle className="w-5 h-5 mb-2" /><p className="text-2xl font-bold">{summary.criticalCount || 0}</p><p className="text-xs opacity-80">Critical</p></button>
          <button onClick={() => setActiveKpi(activeKpi === 'LOW' ? 'ALL' : 'LOW')} className={'p-4 rounded-2xl bg-gradient-to-br from-yellow-600 to-amber-800 text-white shadow-lg text-left ' + (activeKpi === 'LOW' ? 'ring-4 ring-yellow-300' : '')}><TrendingDown className="w-5 h-5 mb-2" /><p className="text-2xl font-bold">{summary.lowCount || 0}</p><p className="text-xs opacity-80">Low Stock</p></button>
          ...
          <button onClick={() => setActiveKpi('ALL')} className={'p-4 rounded-2xl bg-gradient-to-br from-green-600 to-emerald-800 text-white shadow-lg text-left'}><ShoppingCart className="w-5 h-5 mb-2" /><p className="text-2xl font-bold">{summary.totalQuantity || 0}</p><p className="text-xs opacity-80">Units to Order</p></button>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-neutral-900 rounded-2xl border border-neutral-800"><CheckCircle2 className="w-12 h-12 mx-auto mb-3 text-green-500" /><p className="text-neutral-400">All products are adequately stocked</p></div>
        ) : (
          <div className="bg-neutral-900 rounded-2xl border border-neutral-800 overflow-hidden shadow-xl">
            <table className="w-full">
              <thead className="bg-neutral-800"><tr><th className="text-left p-4 text-neutral-400 text-sm">Product</th><th className="text-right p-4 text-neutral-400 text-sm">Current</th><th className="text-right p-4 text-neutral-400 text-sm">Min</th><th className="text-right p-4 text-neutral-400 text-sm">Suggested</th><th className="text-right p-4 text-neutral-400 text-sm">Value</th><th className="text-left p-4 text-neutral-400 text-sm">Priority</th><th className="text-center p-4 text-neutral-400 text-sm">Actions</th></tr></thead>
              <tbody>
                {filtered.map((r: any) => (
                  <tr key={r.id} className="border-t border-neutral-800 hover:bg-neutral-800/50">
                    ...
                    <td className="p-4"><div className="flex gap-2 justify-center"><button onClick={() => createPO(r.id, r.suggestedOrderQty)} className="px-3 py-1.5 rounded-lg bg-orange-600 text-white text-xs font-bold hover:bg-orange-700" title="Create PO"><Plus className="w-3 h-3" /> PO</button><button onClick={() => downloadPdf(r.id)} className="p-2 rounded-lg bg-blue-900/50 text-blue-300 hover:bg-blue-800" title="PDF"><Printer className="w-4 h-4" /></button><button onClick={() => deletePO(r.id)} disabled={deleting === r.id} className="p-2 rounded-lg bg-red-900/50 text-red-300 hover:bg-red-800 disabled:opacity-50" title="Delete">{deleting === r.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}</button></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
