...
import { 
  Loader2, Package, Warehouse, Search, Brain, LineChart, PieChart, ShoppingCart, Zap, DollarSign, ShieldAlert,
            ...
            {movementAnomalies.length > 0 && (
              <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-6">
                ...
                {movementAnomalies.map((a: any, i: number) => (
                  <div key={i} className="p-3 rounded-xl bg-red-900/20 flex justify-between mb-2"><div><p className="font-bold text-white">{a.productName}</p><p className="text-xs text-neutral-400">Qty: {a.quantity} | Avg: {Math.round(a.avg_qty)}</p></div><span className={'px-2 py-1 rounded-full text-xs font-bold ' + (a.anomalyLevel === 'CRITICAL' ? 'bg-red-900/50 text-red-300' : 'bg-yellow-900/50 text-yellow-300')}>{a.anomalyLevel}</span></div>
                ))}
              </div>
            )}
            {negativeStock.length > 0 && (
              <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-6">
                ...
                {negativeStock.map((n: any, i: number) => (
                  <div key={i} className="p-3 rounded-xl bg-yellow-900/20 flex justify-between mb-2"><p className="font-bold text-white">{n.name}</p><span className="text-red-400 font-bold">{n.quantity}</span></div>
                ))}
              </div>
            )}
            {shrinkage.length > 0 && (
              <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-6">
                ...
                {shrinkage.map((s: any, i: number) => (
                  <div key={i} className="p-3 rounded-xl bg-purple-900/20 flex justify-between mb-2"><p className="font-bold text-white">{s.name}</p><span className="text-red-400 font-bold">{s.totalShrinkage} units</span></div>
                ))}
              </div>
            )}
          </div>
