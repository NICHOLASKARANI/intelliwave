...
import { canSend } from '@/lib/wavecore/procurement-po'
...
export const POST = procurementHandler(async (request: NextRequest, ctx: { params: { id: string } }) => {
  ...
  if (!canSend(po.status)) {
    return NextResponse.json({ error: 'Only APPROVED POs can be sent (current: ' + po.status + ')' }, { status: 409 })
  }

  const r = await pool.query(
    `UPDATE "PurchaseOrder"
     SET status = 'SENT', "sentAt" = NOW(), "sentBy" = $3, "updatedAt" = NOW()
  ...
  await logProcurementEvent(pool, {
    ...
    summary: 'Sent PO ' + po.number + ' to ' + (po.supplierName || 'supplier'),
  })
