...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ reconciliations: [], count: 0 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const statementBalance = parseFloat(body.statementBalance) || 0
    const bookBalance = parseFloat(body.bookBalance) || 0
    ...
    const result = await pool.query(
      ...
      [id, body.bankAccountId || null, statementBalance, bookBalance, difference, status, session.organizationId]
    )
